...
#include <random>
#include <cmath>
#include "../tool/tooling/omp_loop.hpp"
...
void reset_force(simulation& s, OmpLoop& omploop) {
  omploop.parfor(0, s.nbpart, 1, [&](size_t i) {
    s.fx[i] = 0.0;
    s.fy[i] = 0.0;
    s.fz[i] = 0.0;
  });
...
void compute_forces(simulation& s, OmpLoop& omploop) {
  omploop.parfor<std::vector<double> >(
    0, s.nbpart, 1,
    [&](std::vector<double>& tls) {   // before
      tls.resize(s.nbpart * 3, 0.0);  // local fx, fy, fz
    },
    [&](size_t from, std::vector<double>& tls) { // f
      for (size_t to = 0; to < s.nbpart; ++to) {
        if (from == to) continue;
        ...
        tls[to]             += dx * F;   // fx
        tls[s.nbpart + to]  += dy * F;   // fy
        tls[2*s.nbpart + to]+= dz * F;   // fz
      }
    },
    [&](std::vector<double>& tls) { // after
      for (size_t i = 0; i < s.nbpart; ++i) {
        s.fx[i] += tls[i];
        s.fy[i] += tls[s.nbpart + i];
        s.fz[i] += tls[2*s.nbpart + i];
      }
    }
  );
}
...
int main(int argc, char* argv[]) {
  ...
  omploop.setNbThread(std::atof(argv[5]));        // number of threads
  omploop.setGranularity(100);   // optional tuning
